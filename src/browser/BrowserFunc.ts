...
export class BrowserFunc {
    ...
    */
    async getDashboardData(page?: Page): Promise<DashboardData> {
        ...
        try {
            ...
            // OPTIMIZATION 1: Try to get dashboard data directly from global scope first
            try {
                const directData = await target.evaluate(() => (window as any).dashboard);
                if (directData && directData.userStatus) {
                    this.bot.log(this.bot.isMobile, 'GET-DASHBOARD-DATA', 'Retrieved dashboard data directly from window object', 'log');
                    return directData as DashboardData;
                } else {
                    this.bot.log(this.bot.isMobile, 'GET-DASHBOARD-DATA', `Direct dashboard access failed or incomplete: Type=${typeof directData}`, 'warn');
                }
            } catch (err) {
                this.bot.log(this.bot.isMobile, 'GET-DASHBOARD-DATA', `Direct dashboard access threw: ${err}`, 'warn');
            }

            // OPTIMIZATION 2: Try Hidden API (Netsky method) as robust fallback
            try {
                const apiData = await this.getDashboardDataFromApi(target);
                if (apiData && apiData.userStatus) {
                    this.bot.log(this.bot.isMobile, 'GET-DASHBOARD-DATA', 'Retrieved dashboard data via Hidden API', 'log');
                    return apiData;
                }
            } catch (apiErr) {
                this.bot.log(this.bot.isMobile, 'GET-DASHBOARD-DATA', `Hidden API fallback failed: ${apiErr instanceof Error ? apiErr.message : String(apiErr)}`, 'warn');
            }
            ...
            if (!dashboardData) {
                ...
                this.bot.log(this.bot.isMobile, 'GET-DASHBOARD-DATA', `Unable to parse dashboard script. Current Page: "${title}" (${url})`, 'error')
    ...
    */
    ...
    async getDashboardDataFromApi(page: Page): Promise<DashboardData> {
        try {
            ...
            const response = await this.bot.axios.request(config);
            if (response.data && response.data.dashboard) {
                return response.data.dashboard as DashboardData;
            }
            throw new Error('API response missing dashboard data');
        } catch (error) {
            throw error;
        }
    }

    private buildCookieHeader(cookies: Cookie[]): string {
