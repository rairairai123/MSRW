...
export class MobileFlow {
    ...
    async run(
        ...
        try {
            ...
            // Try to get OAuth access token for app-based activities
            // If this fails (503/blocked), we fall back to UA-spoofing only mode
            let accessToken: string | null = null
            let oauthFailed = false

            try {
                accessToken = await this.bot.login.getMobileAccessToken(this.bot.homePage, account.email, account.totp)
            } catch (oauthError) {
                const msg = oauthError instanceof Error ? oauthError.message : String(oauthError)
                this.bot.log(true, 'MOBILE-FLOW', `OAuth token acquisition failed: ${msg}`, 'warn')
                this.bot.log(true, 'MOBILE-FLOW', '⚠️ Falling back to UA-spoofing mode (mobile searches only, app activities skipped)', 'warn')
                oauthFailed = true
            }
            ...
            // Only fetch app earnable points if we have a valid access token
            let appEarnablePoints = { totalEarnablePoints: 0 }
            if (accessToken && !oauthFailed) {
                try {
                    appEarnablePoints = await this.bot.browser.func.getAppEarnablePoints(accessToken)
                } catch (appErr) {
                    this.bot.log(true, 'MOBILE-FLOW', `Failed to get app earnable points: ${appErr instanceof Error ? appErr.message : String(appErr)}`, 'warn')
                }
            }
            ...
            // Do daily check in (requires access token)
            if (this.bot.config.workers.doDailyCheckIn && accessToken && !oauthFailed) {
                await this.bot.activities.doDailyCheckIn(accessToken, data)
            } else if (this.bot.config.workers.doDailyCheckIn && oauthFailed) {
                this.bot.log(true, 'MOBILE-FLOW', 'Skipping daily check-in (OAuth failed, no access token)', 'warn')
            }

            // Do read to earn (requires access token)
            if (this.bot.config.workers.doReadToEarn && accessToken && !oauthFailed) {
                await this.bot.activities.doReadToEarn(accessToken, data)
            } else if (this.bot.config.workers.doReadToEarn && oauthFailed) {
                this.bot.log(true, 'MOBILE-FLOW', 'Skipping read-to-earn (OAuth failed, no access token)', 'warn')
            }
