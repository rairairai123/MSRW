  ...
  async getMobileAccessToken(page: Page, email: string, totpSecret?: string) {
    ...
    while (Date.now() - start < DEFAULT_TIMEOUTS.oauthMaxMs) {
      ...
      if (
        ...
        if (u.searchParams.get("removed") === "true") {
          this.bot.log(
            this.bot.isMobile,
            "LOGIN-APP",
            "Relogin required (oauth20_desktop.srf?removed=true) - Clearing cookies & Retrying...",
            "warn",
          );
          // CRITICAL: Clear cookies to prevent infinite redirect loop
          await page.context().clearCookies();
          await page.goto(this.authBaseUrl, { waitUntil: "domcontentloaded" });
          continue;
        }
        ...
        if (error) {
          this.bot.log(
            this.bot.isMobile,
            "LOGIN-APP",
            `OAuth Error detected: ${error} - Retrying fresh...`,
            "error",
          );
          await page.context().clearCookies();
          await page.goto(this.authBaseUrl, { waitUntil: "domcontentloaded" });
