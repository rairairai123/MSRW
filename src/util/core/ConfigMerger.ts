...
export class ConfigMerger {
    /**
     * Strip JSON comments from JSONC content
     */
    ...
    private static deepMerge(example: unknown, user: unknown): unknown {
        ...
        const merged: Record<string, unknown> = { ...example as Record<string, unknown> } // Start with example schema

        for (const key in user) {
            if (key in example) {
                // Key exists in both - recurse
                merged[key] = this.deepMerge((example as Record<string, unknown>)[key], (user as Record<string, unknown>)[key])
            } else {
    ...
    public static mergeConfigFile(
        ...
        try {
            ...
            const exampleJson = JSON.parse(this.stripJsonComments(exampleContent))
            const userJson = JSON.parse(this.stripJsonComments(userContent))
            ...
            if (newKeys.length > 0) {
                changes.push(`Added ${newKeys.length} new option(s): ${newKeys.join(', ')}`)
            }
     ...
     */
    private static findNewKeys(example: unknown, user: unknown, prefix = ''): string[] {
        ...
        for (const key in exampleObj) {
            ...
            } else if (typeof exampleObj[key] === 'object' && !Array.isArray(exampleObj[key])) {
                newKeys.push(...this.findNewKeys(exampleObj[key], userObj[key], fullKey))
            }
        }
     ...
     */
    private static findPreservedUserKeys(user: unknown, example: unknown, prefix = ''): string[] {
        ...
        for (const key in userObj) {
            ...
            if (key in exampleObj) {
                ...
                if (typeof userVal === 'object' && !Array.isArray(userVal)) {
                    preserved.push(...this.findPreservedUserKeys(userVal, exampleVal, fullKey))
            ...
            if (Array.isArray(exampleJson) && Array.isArray(userJson)) {
                ...
                if (exampleJson.length > 0 && userJson.length > 0) {
                    ...
                    if (newFields.length > 0) {
                        changes.push(`Added ${newFields.length} new field(s) to accounts: ${newFields.join(', ')}`)
